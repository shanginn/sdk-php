...
final class MutexTestCase extends TestCase
{
    ...
    public function testCancellingQueuedLockRemovesWaiterAndNeverAcquires(): void
    {
        $mutex = new Mutex();
        $waiting = null;
        $ran = false;
        ...
        $this->startRoot(static function () use ($mutex, &$waiting, &$ran, &$error): void {
            $mutex->lock();
            $waiting = Workflow::async(static function () use ($mutex, &$ran): void {
                $mutex->lock();
                $ran = true;
            });
            $waiting->catch(static function (\Throwable $reason) use (&$error): void {
                $error = $reason;
            });
        });

        self::assertInstanceOf(CancellationScopeInterface::class, $waiting);
        self::assertSame(1, $this->context->pendingConditionCount());

        $waiting->cancel();
        $this->factory->tick();

        self::assertTrue($waiting->isCancelled());
