<?php
# Generated by the protocol buffer compiler.  DO NOT EDIT!
# source: temporal/api/errordetails/v1/message.proto

namespace GPBMetadata\Temporal\Api\Errordetails\V1;

class Message
{
    public static $is_initialized = false;

    public static function initOnce() {
        $pool = \Google\Protobuf\Internal\DescriptorPool::getGeneratedPool();

        if (static::$is_initialized == true) {
          return;
        }
        \GPBMetadata\Google\Protobuf\Any::initOnce();
        \GPBMetadata\Temporal\Api\Common\V1\Message::initOnce();
        \GPBMetadata\Temporal\Api\Enums\V1\FailedCause::initOnce();
        \GPBMetadata\Temporal\Api\Enums\V1\PBNamespace::initOnce();
        \GPBMetadata\Temporal\Api\Failure\V1\Message::initOnce();
        $pool->internalAddGeneratedFile(
            '
—
*temporal/api/errordetails/v1/message.prototemporal.api.errordetails.v1$temporal/api/common/v1/message.proto(temporal/api/enums/v1/failed_cause.proto%temporal/api/enums/v1/namespace.proto%temporal/api/failure/v1/message.proto"B
NotFoundFailure
current_cluster (	
active_cluster (	"R
&WorkflowExecutionAlreadyStartedFailure
start_request_id (	
run_id (	"_
NamespaceNotActiveFailure
	namespace (	
current_cluster (	
active_cluster (	"0
NamespaceUnavailableFailure
	namespace (	"¦
NamespaceInvalidStateFailure
	namespace (	4
state (2%.temporal.api.enums.v1.NamespaceState=
allowed_states (2%.temporal.api.enums.v1.NamespaceState"-
NamespaceNotFoundFailure
	namespace (	"
NamespaceAlreadyExistsFailure"k
 ClientVersionNotSupportedFailure
client_version (	
client_name (	
supported_versions (	"d
 ServerVersionNotSupportedFailure
server_version (	(
 client_supported_server_versions (	"%
#CancellationAlreadyRequestedFailure"G
QueryFailedFailure1
failure (2 .temporal.api.failure.v1.Failure")
PermissionDeniedFailure
reason (	"–
ResourceExhaustedFailure<
cause (2-.temporal.api.enums.v1.ResourceExhaustedCause<
scope (2-.temporal.api.enums.v1.ResourceExhaustedScope"v
SystemWorkflowFailureE
workflow_execution (2).temporal.api.common.v1.WorkflowExecution
workflow_error (	"
WorkflowNotReadyFailure"3
NewerBuildExistsFailure
default_build_id (	"Ù
MultiOperationExecutionFailure^
statuses (2L.temporal.api.errordetails.v1.MultiOperationExecutionFailure.OperationStatusW
OperationStatus
code (
message (	%
details (2.google.protobuf.Any"R
&ActivityExecutionAlreadyStartedFailure
start_request_id (	
run_id (	"X
,NexusOperationExecutionAlreadyStartedFailure
start_request_id (	
run_id (	B§
io.temporal.api.errordetails.v1BMessageProtoPZ/go.temporal.io/api/errordetails/v1;errordetailsªTemporalio.Api.ErrorDetails.V1ê!Temporalio::Api::ErrorDetails::V1bproto3'
        , true);

        static::$is_initialized = true;
    }
}

