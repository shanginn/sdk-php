...
final class Mutex
{
    ...
    public function lock(): self
    {
        ...
        try {
            // The ticket condition belongs to the current cancellation scope.
            // Only the oldest waiter is released, so scheduler callback order
            // cannot reverse mutex acquisition order.
            Workflow::await(
            ...
            if (!$acquired && $wasFirst && !$this->locked) {
                // A cancelled first waiter must immediately expose the next
                // ticket instead of leaving it dormant until another command.
                $context = Workflow::getCurrentContext();
                $context instanceof WorkflowContext and $context->resolveConditions();
            }
        }
    }
