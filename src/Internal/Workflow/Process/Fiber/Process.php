 ...
 */
class Process extends FiberScope implements ProcessInterface
{
    public function __construct(
        ...
        $wfInstance->setUpdateExecutor(function (UpdateInput $input, callable $handler, DeferredPromise $resolver) use ($inboundPipeline): void {
            try {
                ...
                $scope->startUpdate(
                    handler: static function () use ($handler, $inboundPipeline, $input): mixed {
        ...
        $wfInstance->getSignalQueue()->onSignal(
            function (string $name, callable $handler, EncodedValues $arguments) use ($inboundPipeline): void {
                // Define Context for interceptors Pipeline
                Workflow::setCurrentContext($this->scopeContext);
     ...
     */
    public function start(MethodHandler|\Closure $handler, array $args = [], bool $deferred = false): void
    {
        try {
            $this->makeCurrent();

            // If the handler is a MethodHandler, resolve the arguments
            if ($handler instanceof MethodHandler) {
                // Prepare typed input
                $values = $handler->resolveArguments(EncodedValues::fromValues($args));
                
                // Manage init method
                $this->context->getWorkflowInstance()->init($values);
                
                // Start the handler with resolved arguments
                parent::start($handler, $values, $deferred);
            } else {
                // Start a closure directly
                parent::start($handler, $args, $deferred);
            }
        } catch (\Throwable $e) {
            $this->complete($e);
        } finally {
    ...
    private function logRunningHandlers(string $happened = 'finished'): void
    {
        ...
        if ($warnUpdates !== []) {
            $message = "Workflow `$workflowName` $happened while update handlers are still running. " .
                ...
                'to complete by using `Workflow::await(Workflow::allHandlersFinished(...));`. ' .
        ...
        if ($warnSignals !== []) {
            $message = "Workflow `$workflowName` $happened while signal handlers are still running. " .
                ...
                '`Workflow::await(Workflow::allHandlersFinished(...));`. ' .
